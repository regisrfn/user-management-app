...
export class UserService {
  ...
  public deleteUser(id: string): Promise<HttpResponse | HttpErrorResponse> {
    return this.http.delete<HttpResponse>(`${environment.API_URL}/delete/${id}`).toPromise()
  }

  public getUsers(): Promise<User[] | HttpErrorResponse> {
    return this.http.get<User[]>(`${environment.API_URL}/get`).toPromise()
  }

  public getUserById(id:string): Promise<User | HttpErrorResponse> {
    return this.http.get<User>(`${environment.API_URL}/get/${id}`).toPromise()
  }

  public saveUser(formData: FormData): Promise<User | HttpErrorResponse> {
    return this.http.post<User>(`${environment.API_URL}/save`, formData).toPromise()
  }

  public selectAnUser(email="", username=""): Promise<User | HttpErrorResponse> {
    return this.http.get<User>(`${environment.API_URL}/select?email=${email}&username=${username}`).toPromise()
  }

  public updateUser(user: User): Promise<User | HttpErrorResponse> {
    return this.http.put<User>(`${environment.API_URL}/update`, user).toPromise()
  }

  public updateProfileImage(formData: FormData, id: string): Observable<HttpEvent<User> | HttpErrorResponse> {
